const Projects = () => {
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-4 p-2 ">
        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 dark:bg-zinc-800 space-y-2 gap-2 hover:scale-105 transition-all delay-75 ">
          <span className="text-zinc-800 text-md font-semibold dark:text-zinc-300">
            Moto Serv
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, Inertia.js, React.js, Tailwind CSS, and
              Laravel Reverb
            </p>
          </span>
          <div className="flex items-center gap-4">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg"
                alt="React"
                className="w-10 h-10"
              />
              <img
                src="https://avatars.githubusercontent.com/u/47703742"
                alt="Inertia.js"
                className="w-10 h-10"
              />
              <img
                src="https://tailwindcss.com/_next/static/media/tailwindcss-mark.d52e9897.svg"
                alt="Tailwind CSS"
                className="w-10 h-10"
              />
              <img
                src="https://raw.githubusercontent.com/laravel/reverb/main/art/logo.svg"
                alt="Laravel Reverb"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 dark:bg-zinc-800 space-y-2 gap-2 hover:scale-105 transition-all delay-75 ">
          <span className="text-zinc-800 text-md font-semibold dark:text-zinc-300">
            JLPT Application System
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, Blade, Bootstrap CSS, and MySQL
            </p>
          </span>
          <div className=" flex items-center gap-4">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg"
                alt="Bootstrap"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"
                alt="MySQL"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nginx/nginx-original.svg"
                alt="Nginx"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 dark:bg-zinc-800 space-y-2 gap-2 hover:scale-105 transition-all delay-75 ">
          <span className="text-zinc-800 text-md font-semibold dark:text-zinc-300">
            Human Resources Information System V1
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, Laravel Blade, Bootstrap, and MySQL
            </p>
          </span>
          <div className="flex items-center gap-4">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg"
                alt="Bootstrap"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"
                alt="MySQL"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 dark:bg-zinc-800 space-y-2 gap-2 hover:scale-105 transition-all delay-75 ">
          <span className="text-zinc-800 text-md font-semibold dark:text-zinc-300">
            Human Resources Information System V2
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, Inertia.js, Vue 3, Tailwind CSS, and MySQL
            </p>
          </span>
          <div className=" flex items-center gap-4">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vuejs/vuejs-original.svg"
                alt="Vue.js"
                className="w-10 h-10"
              />
              <img
                src="https://avatars.githubusercontent.com/u/47703742"
                alt="Inertia.js"
                className="w-10 h-10"
              />
              <img
                src="https://tailwindcss.com/_next/static/media/tailwindcss-mark.d52e9897.svg"
                alt="Tailwind CSS"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"
                alt="MySQL"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 dark:bg-zinc-800 space-y-2 gap-2 hover:scale-105 transition-all delay-75 ">
          <span className="text-zinc-800 text-md font-semibold dark:text-zinc-300">
            Inventory System V1
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, Blade, Bootstrap CSS, and MySQL
            </p>
          </span>
          <div className=" flex items-center gap-4">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg"
                alt="Bootstrap"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"
                alt="MySQL"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/inkscape/inkscape-original.svg"
                alt="Inkscape"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nginx/nginx-original.svg"
                alt="Nginx"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 dark:bg-zinc-800 space-y-2 gap-2 hover:scale-105 transition-all delay-75 ">
          <span className="text-zinc-800 text-md font-semibold dark:text-zinc-300">
            Inventory System V2
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, Inertia.js, Vue 3, Tailwind CSS, and MySQL
            </p>
          </span>
          <div className=" flex items-center gap-4">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vuejs/vuejs-original.svg"
                alt="Vue.js"
                className="w-10 h-10"
              />
              <img
                src="https://avatars.githubusercontent.com/u/47703742"
                alt="Inertia.js"
                className="w-10 h-10"
              />
              <img
                src="https://tailwindcss.com/_next/static/media/tailwindcss-mark.d52e9897.svg"
                alt="Tailwind CSS"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"
                alt="MySQL"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg"
                alt="Figma"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 dark:bg-zinc-800 space-y-2 gap-2 hover:scale-105 transition-all delay-75 ">
          <span className="text-zinc-800 text-md font-semibold dark:text-zinc-300">
            Announcement System V2
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, Inertia.js, Vue 3, Tailwind CSS, and MySQL
            </p>
          </span>
          <div className=" flex items-center gap-4">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg"
                alt="React"
                className="w-10 h-10"
              />
              <img
                src="https://avatars.githubusercontent.com/u/47703742"
                alt="Inertia.js"
                className="w-10 h-10"
              />
              <img
                src="https://tailwindcss.com/_next/static/media/tailwindcss-mark.d52e9897.svg"
                alt="Tailwind CSS"
                className="w-10 h-10"
              />
              <img
                src="https://raw.githubusercontent.com/laravel/reverb/main/art/logo.svg"
                alt="Laravel Reverb"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nginx/nginx-original.svg"
                alt="Nginx"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 dark:bg-zinc-800 space-y-2 gap-2 hover:scale-105 transition-all delay-75 ">
          <span className="text-zinc-800 text-md font-semibold blur-xs">
            Casino Back Office
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, React.js, Material UI, RabbitMQ
            </p>
          </span>
          <div className=" flex items-center gap-4 blur-xs">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg"
                alt="React"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/materialui/materialui-original.svg"
                alt="Material UI"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/rabbitmq/rabbitmq-original.svg"
                alt="RabbitMQ"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 space-y-2 gap-2 hover:scale-105 transition-all delay-75 dark:bg-zinc-800">
          <span className="text-zinc-800 text-md font-semibold blur-xs">
            Casino Client Side
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, React.js, Material UI, RabbitMQ
            </p>
          </span>
          <div className=" flex items-center gap-4 blur-xs">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg"
                alt="React"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/materialui/materialui-original.svg"
                alt="Material UI"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/rabbitmq/rabbitmq-original.svg"
                alt="RabbitMQ"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 space-y-2 gap-2 hover:scale-105 transition-all delay-75 dark:bg-zinc-800">
          <span className="text-zinc-800 text-md font-semibold blur-xs">
            Casino Agent Side
            <p className="text-zinc-500 text-sm font-normal">
              Built with Laravel, React.js, Vite.js, Go, MySQL
            </p>
          </span>
          <div className=" flex items-center gap-4 blur-xs">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://raw.githubusercontent.com/laravel/art/master/laravel-logo.png"
                alt="Laravel"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg"
                alt="React"
                className="w-10 h-10"
              />
              <img
                src="https://vitejs.dev/logo.svg"
                alt="Vite"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg"
                alt="Go"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"
                alt="MySQL"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>

        <div className="flex flex-col p-5 rounded-md drop-shadow-xl bg-zinc-100 space-y-2 gap-2 hover:scale-105 transition-all delay-75 dark:bg-zinc-800">
          <span className="text-zinc-800 text-md font-semibold blur-xs">
            Casino Reporting Service
            <p className="text-zinc-500 text-sm font-normal">
              Built with Go, Gin, GORM, MySQL
            </p>
          </span>
          <div className=" flex items-center gap-4 blur-xs">
            <div className="flex justify-center flex-row gap-2">
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg"
                alt="Go"
                className="w-10 h-10"
              />
              <img
                src="https://raw.githubusercontent.com/gin-gonic/logo/master/color.png"
                alt="Gin"
                className="w-10 h-10"
              />
              <img
                src="https://gorm.io/gorm.svg"
                alt="GORM"
                className="w-10 h-10"
              />
              <img
                src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"
                alt="MySQL"
                className="w-10 h-10"
              />
            </div>
          </div>
        </div>
      </div>
    </>
  )
}

export default Projects
